...
function Header(props) {
  ...
  return (
    <Navbar expand='lg' sticky='top' className='header'>
          ...
          <Navbar.Toggle  onClick={()=>setCollaspse(!collapse)} >{collapse ? <CloseIcon /> : <MenuIcon/>}</Navbar.Toggle>

      <Navbar.Collapse>
        <Nav className='header_left'>
          <Nav.Link as={NavLink} key="Resume" to='/'
           className={pathname === '/' ? 'header_link_active' :'header_link'}
          >
             Resume
          </Nav.Link>
          <Nav.Link as={NavLink} key="Portfolio"  to='/Portfolio' 
